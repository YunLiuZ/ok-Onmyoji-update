...
- 还有一件事，锁定阵容参数比较特殊，当你不选的时候默认会锁定阵容进入战斗，当选上后，第一次战斗会切换阵容，第二次战斗锁定阵容，所以当你想切换阵容的时候可以考虑将这个参数勾选上，还有一件事，强烈建议自己刷一次后锁定阵容，尽量少用这个参数，这个辅助如果你只是当作减少反复刷副本的次数时它应该会很好用，对于一些特殊情况作者也尽量的处理了，也会经常出现意想不到的情况，所以尽量让辅助只做重复的事，当然这些功能作者反复测过很多次，基本上没有什么问题
